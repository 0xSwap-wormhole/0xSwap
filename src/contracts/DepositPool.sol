...
contract DepositPool is IDepositPool, IERC20 {
    ...
    constructor(address _uniRouter, address _uniPair, address _token0, address _token1, address _positionManager) {
        ...
        uniPair = _uniPair;
        (token0, token1) = 0xswapLibrary.sortTokens(_token0, _token1);
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = 0xswapLibrary.getUniReserves(uniPair, token0, token1);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = 0xswapLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'DepositPool: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = 0xswapLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    */
    ...
    function addBorrowedTokens(uint256 tokensOwed0, uint256 tokensOwed1, uint256 liquidity) internal {
        ...
        uint256 borrowedLiquidity = 0xswapLibrary.convertAmountsToLiquidity(tokensOwed0, tokensOwed1);
    ...
    function mintDevFee(uint256 nextBorrowedInvariant, uint256 _lastFeeIndex) private {
        ...
        if(feeTo != address(0)) {
            ...
            (uint256 reserve0, uint256 reserve1) = 0xswapLibrary.getUniReserves(uniPair, token0, token1);
            uint256 rootK = 0xswapLibrary.convertAmountsToLiquidity(reserve0, reserve1);
    ...
    function getLastFeeIndex() external override view returns(uint _accFeeIndex, uint _lastUniInvariant, uint _lastUniTotalSupply, uint _lastFeeIndex) {
        ...
        if(lastUniInvariant > 0 && _lastUniInvariant > 0 && _lastUniTotalSupply > 0 && lastUniTotalSupply > 0) {
            uint256 numerator =  0xswapLibrary.rootNumber(_lastUniInvariant) * lastUniTotalSupply;
            uint256 denominator =  0xswapLibrary.rootNumber(lastUniInvariant) * _lastUniTotalSupply;
    ...
    function getBorrowedReserves() external view returns(uint256 _reserve0, uint256 _reserve1) {
        (uint256 _uniReserve0, uint256 _uniReserve1) = 0xswapLibrary.getUniReserves(uniPair, token0, token1);
        (_reserve0, _reserve1) = 0xswapLibrary.getBorrowedReserves(uniPair, _uniReserve0, _uniReserve1, totalUniLiquidity, BORROWED_INVARIANT);
    }
    ...
    //function mint(address to) external lock returns (uint256 liquidity) {
        (uint amount0, uint amount1) = 0xswapLibrary.getTokenBalances(token0, token1, address(this));
        ...
        if(_totalSupply > 0) {
            ...
            (_reserve0, _reserve1) = 0xswapLibrary.getBorrowedReserves(uniPair, _uniReserve0, _uniReserve1, totalUniLiquidity, BORROWED_INVARIANT);//This is right because the liquidity has to
        }
        ...
        if (_totalSupply == 0) {
            liquidity = 0xswapLibrary.convertAmountsToLiquidity(amountA, amountB) - MINIMUM_LIQUIDITY;
            _mint(address(0), MINIMUM_LIQUIDITY); // permanently lock the first MINIMUM_LIQUIDITY tokens
        } else {
            liquidity = 0xswapLibrary.min2((amountA * _totalSupply) / _reserve0, (amountB * _totalSupply) / _reserve1);//match everything that is in uniswap
        }
    ...
    function calculateLiquidityAmounts(uint liquidity) internal returns(uint256 amount0, uint256 amount1) {
        (uint balance0, uint balance1) = 0xswapLibrary.getTokenBalances(token0, token1, uniPair);

        (uint256 _reserve0, uint256 _reserve1) = 0xswapLibrary.getBorrowedReserves(uniPair, balance0, balance1, totalUniLiquidity, BORROWED_INVARIANT);
    ...
    function calculateUniLiquidity(uint amount0, uint amount1) internal returns(uint256 uniLiquidity) {
        ...
        (uint256 _uniReserve0, uint256 _uniReserve1) = 0xswapLibrary.getUniReserves(uniPair, token0, token1);
        uint256 uniTotalSupply = IERC20(uniPair).totalSupply();
        uniLiquidity = 0xswapLibrary.min2((amount0 * uniTotalSupply) / _uniReserve0, (amount1 * uniTotalSupply) / _uniReserve1);
    }
    ...
    function closePosition(uint256 liquidity) external override returns(uint256 leftAmount0, uint256 leftAmount1) {
        ...
        (uint256 amount0, uint256 amount1) = 0xswapLibrary.convertPoolLiquidityToAmounts(uniPair, liquidity);
        ...
        //All amounts are transferred to positionManager
        (leftAmount0, leftAmount1) = 0xswapLibrary.getTokenBalances(token0, token1, address(this));
