...
library 0xswapPosLibrary {
    ...
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, '0xswapPosLibrary: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), '0xswapPosLibrary: ZERO_ADDRESS');
    }
    ...
    function checkCollateral(IPositionManager.Position storage position, uint16 limit) internal {
        ...
        require((heldBalance * limit) / 1000 >= owedBalance, '0xswapPosLibrary: INSUFFICIENT_COLLATERAL_DEPOSITED');
    }
