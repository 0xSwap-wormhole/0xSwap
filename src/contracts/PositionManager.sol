...
import './libraries/0xswapPosLibrary.sol';
...
contract PositionManager is IPositionManager, ERC721 {
    ...
    function updateTokenBalances(Position storage position) internal {
        (_tokenBalances[position.token0], _tokenBalances[position.token1]) = 0xswapPosLibrary.getTokenBalances(position.token0, position.token1, address(this));
    ...
    function mint(address token0, address token1, uint256 liquidity, address to) internal returns (uint256 tokenId) {
        ...
        //(address _token0, address _token1) = 0xswapPosLibrary.sortTokens(params.token0, params.token1);
        (address _token0, address _token1) = 0xswapPosLibrary.sortTokens(token0, token1);
        ...
        uint256 _liquidity = 0xswapPosLibrary.convertAmountsToLiquidity(_tokensOwed0,_tokensOwed1);//this liquidity
        ...
        0xswapPosLibrary.checkCollateral(position, 750);
    ...
    function decreaseCollateral(uint256 tokenId, uint256 amount0, uint256 amount1, address to) external isAuthorizedForToken(tokenId) returns (uint256 tokensHeld0, uint256 tokensHeld1)
    {
        ...
        (, uint256 liquidity) = 0xswapPosLibrary.getPositionLiquidity(position);//We store the interest charged as added liquidity. To do that we have to square the cumRate
        (uint256 owedBalance, uint256 heldBalance) = 0xswapPosLibrary.getPositionBalances(position.uniPair, liquidity, position.tokensHeld0, position.tokensHeld1);//, position.uniPairHeld);
    ...
    function updatePositionLiquidity(Position storage position) internal {
        (position.rateIndex, position.liquidity) = 0xswapPosLibrary.getPositionLiquidity(position);
    }
    ...
    function decreasePosition(uint256 tokenId, uint256 liquidity) external isAuthorizedForToken(tokenId) {
        ...
        uint256 totalLiquidityProvided = 0xswapPosLibrary.convertAmountsToLiquidity(position.tokensHeld0, position.tokensHeld1);
        ...
        (uint256 owedBalance, uint256 heldBalance) = 0xswapPosLibrary.getPositionBalances(position.uniPair, position.liquidity, position.tokensHeld0, position.tokensHeld1);//, position.uniPairHeld);
    ...
    /*function swapPositionExactTokensForTokens(RebalanceParams calldata params) external isAuthorizedForToken(params.tokenId) {
        ...
        0xswapPosLibrary.checkCollateral(position, 850);
